...
class Source(object):
    ...
    def __init__(self, path, remove_bad=True, verbose=True,
        ...
        self.cutout_data = CutoutData(
            path, remove_bad, verbose, self.provenance,
            quality, bkg_subtract, bkg_n
        )
    ...
    def add_cpm_model(self, exclusion_size=5,
        ...
        if self.models is None:
            LOGINFO("Please set the aperture first.")
    ...
    def holdout_fit_predict(self, k=10, mask=None, verbose=False):
        ...
        if self.models is None:
            LOGINFO("Please set the aperture first.")
        if mask is not None:
            LOGINFO(f"Using user-provided mask. Clipping "
        ...
        for row_models in self.models:
            ...
            for model in row_models:
                times, flux, pred = model.holdout_fit_predict(
                    k, mask, verbose=verbose
                )
    ...
    def plot_cutout(self, rowlims=None, collims=None, l=10, h=90,
                    show_aperture=False, projection=None, figpath=None):
        ...
        if rowlims is not None:
            plt.yticks(np.arange(rowlims[-1]+1-rowlims[0]),
                       labels=[str(i) for i in np.arange(rows[0], rows[-1]+1)])

        if collims is not None:
            plt.xticks(np.arange(collims[-1]+1-collims[0]),
                       labels=[str(i) for i in np.arange(cols[0], cols[-1]+1)])

        if show_aperture:
            aperture = self.aperture[
                rows[0]:rows[-1]+1, cols[0]:cols[-1]+1
            ]
            plt.imshow(np.ma.masked_where(aperture == False, aperture),
    ...
    def plot_pix_by_pix(self, data_type="raw", split=False,
                        show_locations=True, show_labels=True, fontsize=15,
                        figsize=(12, 8), thin=1, marker=".", ms=1,
        ...
        for r in rows:
            for c in cols:
                # Flip the rows so that they match origin='lower' setting
                ax = axs[rows[-1] - r, c]
                if split:
                ...
                if show_locations:
                    ax.text(x=0.98, y=0.98,
                            s=f"[{self.models[r][c].row},{self.models[r][c].col}]",
                ...
                if show_labels:
                    ...
                    fig.text(x=ylabel_xloc, y=0.5, s=y_label,
                             fontsize=fontsize, rotation="vertical",
                             va="center", rasterized=False)
                    fig.text(x=0.5, y=0.06, s="Time [BJD- 2457000]",
                             fontsize=fontsize, ha="center", rasterized=False)
    ...
    def make_animation(self, data_type="cpm_subtracted_flux", l=0, h=100, thin=5):
        ...
        for i in range(0, lc_matrix.shape[0], thin):
            # origin="lower" is not used 
            im1 = axes.imshow(lc_matrix[i], animated=True,
                              vmin=np.percentile(lc_matrix, l),
                              vmax=np.percentile(lc_matrix, h))
            ims.append([im1])
    ...
    def get_outliers(self, data_type="cpm_subtracted_flux", window=50, sigma=5,
                     sigma_upper=None, sigma_lower=None):
    ...
    def get_aperture_lc(self, data_type="raw", weighting=None, split=False, verbose=True):
        ...
        if verbose:
            LOGINFO(f"Summing over {rows.size} x {cols.size} pixel "
                    f"lightcurves. Weighting={weighting}")
        if split:
    ...
    def calc_min_cpm_reg(self, cpm_regs, k, mask=None, figpath=None, **kwargs):
        ...
        for idx, reg in enumerate(cpm_regs):
            ...
            apt_cpm_subtracted_lc = self.get_aperture_lc(
                split=True, data_type="cpm_subtracted_flux", verbose=False
            )
            split_cdpp = np.array(
                [self._calc_cdpp(flux, **kwargs) for flux in apt_cpm_subtracted_lc]
            )
        ...
        if figpath is not None:
            ...
            if k <= 10:
                ...
                txt = f'Lines are {k} different sections.'
